...
interface ClaudeAgentEmbeddedProps {
  figmaUrl: string;
  projectName: string;
  githubUrl?: string;
}

export default function ClaudeAgentEmbedded({ 
  figmaUrl, 
  projectName,
  githubUrl: initialGithubUrl 
}: ClaudeAgentEmbeddedProps) {
  const [sessionId, setSessionId] = useState<string | null>(null);
  const [messages, setMessages] = useState<Array<{ role: string; content: string }>>([]);
  const [inputMessage, setInputMessage] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [githubUrl, setGithubUrl] = useState<string | null>(initialGithubUrl || null);
  ...
  const startNewSession = async () => {
    ...
    // 모달이 열릴 때 현재 Figma URL을 기본값으로 설정
    setGithubRepoUrl(localStorage.getItem("claude_agent_github_url") || "");
  };
    ...
    <div className="flex flex-col h-[calc(100vh-200px)] bg-gray-50">
      ...
      <div className="bg-white border-b p-4">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-bold">Claude Agent System</h2>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col bg-white">
          <div className="p-4 border-b bg-blue-50">
            <h3 className="font-semibold text-blue-900">클로드1 (작업자)</h3>
            {githubUrl && (
              <a
                href={githubUrl}
                target="_blank"
        ...
        <div className="w-96 bg-gray-100 border-l flex flex-col">
          <div className="p-4 border-b bg-green-50">
            <h3 className="font-semibold text-green-900">클로드2 (감독자)</h3>
          </div>
